...
- `immutable: true` 설정 시 수정 불가 → 실수 방지, 성능 향상 (삭제 후 재생성만 가능)
...
# stringData: 평문으로 작성 → 적용 시 자동 Base64 인코딩
# stringData:
#   username: admin
#   password: S3cur3P@ss!
```
...
ConfigMap과 동일한 방식 (환경 변수, 볼륨 마운트)으로 사용. `configMapKeyRef` 대신 `secretKeyRef` 사용.
...
보안 강화 방법:
  ...
  3. 외부 시크릿 관리자 연동 (Vault, AWS Secrets Manager, Sealed Secrets 등)
```
...
### "가용(Available)"의 판단 기준 — readinessProbe

롤링 업데이트에서 **Pod가 "가용"으로 카운트되려면 readinessProbe를 통과**해야 함
...
readinessProbe 실행
    │
    ├─ 실패 → "가용하지 않음" → maxUnavailable 카운트에 포함
    │         Service 엔드포인트에도 미등록 (트래픽 안 감)
    │
    └─ 성공 → minReadySeconds 경과 확인
                │
                ├─ 미경과 → 아직 "가용하지 않음"
                │
                └─ 경과 → "가용(Available)" 판정
...
```
replicas=3, maxSurge=1, maxUnavailable=0 + readinessProbe 설정 시:

Step 1: [v1✅] [v1✅] [v1✅]           가용=3, 최소 가용=3 ✅
Step 2: [v1✅] [v1✅] [v1✅] [v2⏳]    v2 생성, readinessProbe 아직 미통과
                                       가용=3 (v2는 미카운트)
Step 3: [v1✅] [v1✅] [v1✅] [v2✅]    v2 readinessProbe 통과!
                                       가용=4 → 이제 v1 하나 종료 가능
Step 4: [v1✅] [v1✅] [v1🔻] [v2✅]    v1 하나 종료 시작
...
```

| 상황 | 가용 판정 |
| --- | --- |
| readinessProbe 미설정 | 컨테이너 Running 즉시 가용 |
| readinessProbe 성공 | 가용 (+ minReadySeconds 대기 후) |
| readinessProbe 실패 | 가용하지 않음 → 롤링 업데이트 진행 멈춤 |
| readinessProbe 계속 실패 | `progressDeadlineSeconds`(기본 600초) 초과 시 배포 실패 |

- readinessProbe가 롤링 업데이트의 **안전장치** 역할
- 새 버전에 문제가 있으면 readinessProbe 실패 → 자동으로 배포 중단 → 기존 버전 유지
...
# 배포 일시 중지 / 재개 (여러 변경을 모아서 한 번에 적용 시)
...
- `revisionHistoryLimit` (기본 10): 보관할 이전 ReplicaSet 수. 0이면 롤백 불가
- `minReadySeconds`: 새 Pod가 Ready 후 추가 대기 시간 (안정성 확인)
- `progressDeadlineSeconds` (기본 600초): 배포가 진행되지 않으면 실패로 판단
...
| 기능 | 설명 |
| --- | --- |
| `namePrefix` / `nameSuffix` | 리소스 이름에 접두사/접미사 |
| `namespace` | 네임스페이스 일괄 지정 |
| `commonLabels` | 모든 리소스에 레이블 추가 |
| `images` | 이미지 이름/태그 변경 |
| `configMapGenerator` | ConfigMap 자동 생성 (해시 suffix → 변경 시 Pod 자동 재시작) |
...
my-chart/
 ...
 ├─ values.yaml             # 기본 설정값
 ├─ templates/              # Kubernetes 매니페스트 템플릿 (Go template)
 │   ├─ deployment.yaml
 │   ├─ service.yaml
...
**values.yaml — 사용자가 오버라이드하는 기본 설정값**
```yaml
replicaCount: 3
image:
  repository: my-app
...
| 패키지/릴리스 관리 | 없음 | Chart 리포지토리, 버전 관리, 롤백 |
...
```bash
# 명령형: CPU 사용률 50% 기준, 최소 2개 ~ 최대 10개
...
- **전제: Pod에 resources.requests가 반드시 설정**되어 있어야 함 (requests 대비 사용률 계산)
...
```
scaleUp: stabilizationWindowSeconds: 0 → 부하 증가에 즉시 반응
scaleDown: stabilizationWindowSeconds: 300 → 5분 대기 후 축소
  → "플래핑(flapping)" 방지 (스케일 아웃/인이 빈번하게 반복되는 현상)
```
