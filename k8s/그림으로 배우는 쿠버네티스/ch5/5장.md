...
쿠버네티스와 스토리지 시스템 간의 표준 인터페이스

```
┌─────────────────────────────────────────────────┐
│                  Kubernetes                      │
│  PVC ─→ StorageClass ─→ CSI Driver ─→ Storage   │
└─────────────────────────────────────────────────┘
                           │
              ┌────────────┼────────────┐
              ▼            ▼            ▼
...
**배경**
- 기존에는 쿠버네티스 코어에 스토리지 드라이버가 내장됨 (in-tree)
- 새 스토리지 지원 시 쿠버네티스 자체를 수정해야 하는 문제
- CSI로 스토리지 드라이버를 플러그인 형태로 분리 (out-of-tree)

**in-tree vs CSI 드라이버**
| 구분 | in-tree (레거시) | CSI (현재 표준) |
| --- | --- | --- |
| AWS EBS | `kubernetes.io/aws-ebs` | `ebs.csi.aws.com` |
| GCP PD | `kubernetes.io/gce-pd` | `pd.csi.storage.gke.io` |
| Azure Disk | `kubernetes.io/azure-disk` | `disk.csi.azure.com` |
| 업데이트 | 쿠버네티스 릴리스에 종속 | 독립적으로 업데이트 가능 |

**CSI 드라이버 구성 요소**
- **Controller Plugin**: PV 생성/삭제/스냅샷 (보통 Deployment)
- **Node Plugin**: 노드에 볼륨 마운트/언마운트 (DaemonSet)
...
**특징**
- 스토리지 벤더가 독립적으로 드라이버 개발/배포
- 스냅샷, 클로닝, 볼륨 확장 등 고급 기능 표준화
- 쿠버네티스 1.13+에서 GA (권장 방식)

## Volume Snapshot

PVC의 특정 시점 스냅샷을 생성하여 백업/복구에 활용

```
PVC (data) ─── snapshot ───→ VolumeSnapshot
                                   │
                                   ▼
                            VolumeSnapshotContent (실제 스냅샷)
                                   │
                            restore ↓
                            New PVC (data-restored)
```

**구성 요소**
- **VolumeSnapshotClass**: 스냅샷 프로비저너 정의 (StorageClass와 유사)
- **VolumeSnapshot**: 사용자가 요청하는 스냅샷 (PVC와 유사)
- **VolumeSnapshotContent**: 실제 스냅샷 리소스 (PV와 유사)
...
**사용 예**
- DB 업그레이드 전 데이터 백업
- 운영 데이터를 개발/테스트 환경으로 복제
- 장애 복구를 위한 정기 스냅샷 (CronJob + VolumeSnapshot)
- 데이터 마이그레이션 시 롤백 지점 확보

**주의사항**
- CSI 드라이버가 스냅샷 기능을 지원해야 함
- 스냅샷은 crash-consistent (앱 레벨 일관성은 별도 처리 필요)
- 스냅샷 저장 비용 발생 (클라우드 환경)
