...
Redis 서버가 확률분포에 기반한 알고리즘을 통해 TTL 값을 미리 갱신 시켜주는 방식입니다. 이를 PER(Probabilistic Early Recomputation)라고 합니다.

## 세션 스토어로서의 Redis

### 세션이란?

* 세션은 서비스를 사용하는 클라이언트의 상태 정보를 의미합니다.

### 세션 스토어가 필요한 이유

* 하나의 서비스가 여러 서버를 통해 관리가 될 때, 서버별로 세션 스토어를 따로 관리한다면 클라이언트는 클라이언트의 세션 정보를 가지는 서버에 종속됩니다. 이를 sticky session이라고 합니다.
* 하지만 sticky session은 서버의 확장성을 떨어뜨리는 단점이 있습니다. 따라서 클라이언트의 세션 정보를 모든 서버에 저장하는 방식인 all-to-all 방법을 통해 해결할 수 있습니다.
* all-to-all 방법은 클라이언트를 여러 서버에 분산시킬 수 있지만, 클라이언트의 세션 데이터는 여러 서버로 복사돼 저장되기 때문에 불필요한 저장 공간을 차지하게 됩니다. 
또한 복제하는 과정에서 불필요한 네트워크 트래픽도 다수 발생하게 됩니다.
* 이러한 문제를 Redis를 세션 스토어로 사용함으로써 해결할 수 있습니다.
* Redis는 메모리 기반의 데이터 스토어이기 때문에 데이터를 빠르게 읽고 쓸 수 있습니다. 따라서 클라이언트의 세션 데이터를 빠르게 읽고 쓸 수 있습니다.

### 캐시와 세션의 차이

* Redis를 캐시로 사용할 때는 다음과 같은 과정을 거칩니다.
  1. 애플리케이션에서 데이터를 조회합니다.
  2. Redis에 데이터가 존재하는지 확인합니다.
  3. Redis에 데이터가 존재한다면 Redis에서 데이터를 조회합니다.
  4. Redis에 데이터가 존재하지 않는다면 원본 데이터 저장소에서 데이터를 조회합니다.
* 캐시는 데이터베이스의 완벽한 서브셋으로 동작합니다.
* 세션 스토어에 저장된 데이터는 여러 클라이언트 간 공유되지 않으며, 특정 사용자 ID에 한해 유효합니다.
* 세션이 활성화돼 있는 동안에는 애플리케이션은 클라이언트의 데이터를 데이터베이스가 아닌 세션 스토어에만 저장합니다.
* 세션이 비활성화가 되면 애플리케이션 처리에 따라 세션 스토어에 저장된 데이터를 데이터베이스에 저장할 수도 있고, 삭제할 수도 있습니다.
* 세션 스토어는 데이터베이스의 완벽한 서브셋이 아니기에 세션 스토어에 장애가 발생한다면 데이터가 유실될 수 있습니다.